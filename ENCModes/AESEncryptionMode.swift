...
public class AESEncryptionMode : EncryptionMode {
    ...
    private(set) public var sessionEncryptionKey: [UInt8]
    private(set) public var sessionMacKey: [UInt8]
    ...
    init(communicator: DnaCommunicator, key: [UInt8], challengeA: [UInt8], challengeB: [UInt8]) {
        self.communicator = communicator
        self.sessionEncryptionKey = AESEncryptionMode.generateAESSessionKey(key: key, purpose: AESEncryptionMode.encryptionPurpose, challengeA: challengeA, challengeB: challengeB)
        self.sessionMacKey = AESEncryptionMode.generateAESSessionKey(key: key, purpose: AESEncryptionMode.macPurpose, challengeA: challengeA, challengeB: challengeB)
        
        Helper.logBytes("Session Enc Key: ", sessionEncryptionKey)
        Helper.logBytes("Session Mac Key: ", sessionMacKey)
        keyDataClass.shared.macKey = sessionMacKey
        keyDataClass.shared.macENCKey = sessionEncryptionKey
    }
    ...
    func encryptData(message: [UInt8]) -> [UInt8] {
        return Helper.simpleAesEncrypt(key: sessionEncryptionKey, data: Helper.messageWithPadding(message), iv: generateIv(purpose: AESEncryptionMode.encryptionPurpose))
    }
    
    func decryptData(message: [UInt8]) -> [UInt8] {
        return Helper.simpleAesDecrypt(key: sessionEncryptionKey, data: message, iv: generateIv(purpose: AESEncryptionMode.decryptionPurpose))
    }
    ...
    static func generateAESSessionKey(key: [UInt8], purpose: [UInt8], challengeA: [UInt8], challengeB: [UInt8]) -> [UInt8] {
        let sessionVector = generateAESSessionVector(purpose: purpose, challengeA: challengeA, challengeB: challengeB)
        return Helper.simpleCMAC(key: key, data: sessionVector)
    }
    
    static func generateAESSessionVector(purpose: [UInt8], challengeA: [UInt8], challengeB: [UInt8]) -> [UInt8] {
